...
const Messages: React.FC<MessagesProps> = ({ setChatUser }) => {
  ...
  const messages = [
    { id: 1, name: 'Gabrielzin', lastMessage: 'E ai, gata. tudo bem? Isso é uma conversa longa que será cortada.' },
    { id: 2, name: 'Fela', lastMessage: 'Que delicia hein' },
    { id: 3, name: 'Fulano', lastMessage: 'Como vai?' },
    { id: 4, name: 'Ciclano', lastMessage: 'Tech geek, procurando alguém para trocar ideias.' },
    // Adicione mais mensagens conforme necessário
  ];

  const handleMenuClick = (event: React.MouseEvent<HTMLElement>, userName: string) => {
  ...
  return (
    <div className="messages-container" style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
      ...
      {messages.length === 0 ? (
        <Typography className={classes.noMessagesText}>Nenhuma conversa</Typography>
      ) : (
            ...
            .map((message) => (
              <Grid item xs={12} sm={6} md={4} key={message.id}>
                ...
                >
                  ...
                  <MenuItem onClick={handleMenuClose}>Excluir conversa</MenuItem>
                </Menu>
              </Grid>
            ))}
        </Grid>
